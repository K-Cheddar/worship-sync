...
The script scans media library references, already-owned quota-ledger entries, tracked Mux direct uploads, and Mux assets with verifiable WorshipSync church metadata. This includes a completed direct upload whose browser client closed before the asset poll completed. Temporary conversion assets remain excluded. Cloudinary assets still require a media or ledger reference because the provider metadata is not sufficient to assign every legacy asset safely. These are application-enforced quotas, not comprehensive controls over every object in the provider accounts; unresolved provider assets remain outside church quota ownership until they are reviewed.
