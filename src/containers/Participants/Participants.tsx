...
import { addParticipant, deleteParticipant, selectParticipant, updateParticipant } from "../../store/participantsSlice";
...
const Participants = () => {
  const { list, id, name, title, event, showDelete} = useSelector(state => state.participants)
        ...
        <section className="w-1/2 flex flex-col gap-2 h-full">
          <ul className="participants-list">
            {list.map((participant) => {
              ...
              return (
                <li key={participant.id} className={`flex items-center rounded-lg w-full leading-3 ${isSelected ? 'bg-slate-950' : 'bg-slate-800'}`}>
                  <Button
                    variant="tertiary"
                    wrap 
                  ...
                  >
                    {participant.name && <span className="text-base flex items-center h-full">{participant.name}</span>}
                    {participant.title && <span className="flex text-sm items-center h-full italic">{participant.title}</span>}
                    {participant.event && <span className="flex text-sm items-center h-full">{participant.event}</span>}
                    {!hasData && <span className="text-sm leading-7">Click to add participant details</span>}
                  </Button>
                  ...
                  {hasData && (
                    <Button 
                      ...
                      svg={OverlaysSVG}
                      onClick={() => dispatch(updateOverlayInfo({name: participant.name, title: participant.title, event: participant.event}))}
                    >
                      Send
                    </Button>
                  )}
