...
const SongSections = ({ songOrder, setSongOrder, currentSections } : SongSectionsProps) => {
      ...
      <h2 className="text-lg mb-2 text-center font-semibold">Song Order</h2>
        <ul className="song-sections">
          {songOrder.map(({ id, name }, index) => (
            <li 
              key={id} 
              className={`flex items-center px-2 h-7 bg-black rounded-lg hover:bg-gray-800 cursor-pointer border-b-4 ${songSectionBorderColorMap.get(name.split(' ')[0])}`}>
        ...
        <div className="mt-4">
          <h4 className="text-sm font-semibold">Select Section:</h4>
          <Select className="text-sm" options={currentSections} value={section} onChange={(value) => setSection(value)}/>
          <Button className="text-base mt-2">Add Section</Button>
        </div>
  </>
  )
}
