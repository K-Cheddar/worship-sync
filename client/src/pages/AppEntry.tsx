...
const productHighlights: ProductHighlight[] = [
  ...
  },
  {
    ...
    description:
      "Moderate attendee questions and coordinate with your team between Services.",
    icon: MessagesSquare,
  },
];
...
const AppEntry = () => {
  ...
  const connectionNotice = isAuthConnectionNoticeVisible ? (
    <div
      className="rounded-lg border border-yellow-400/40 bg-yellow-500/10 p-3 text-sm text-yellow-50"
      role="status"
    >
      <p className="font-semibold">{connectionNoticeTitle}</p>
      {connectionNoticeMessage ? (
        <p className="mt-1 text-yellow-100/90">{connectionNoticeMessage}</p>
      ) : null}
  ...
  return (
    <AuthScreenMain extraMenuItems={installMenuItems}>
            ...
            <div className="mt-6 flex flex-col items-center text-center">
              ...
              <p className="mt-2 max-w-2xl text-sm text-gray-200 sm:text-base">
                {isStoredServerSessionBlocked
                  ...
                  : "Presentation, scheduling, chat, and discussion boards—so the room, the stream, and the team stay connected."}
              </p>
            </div>
